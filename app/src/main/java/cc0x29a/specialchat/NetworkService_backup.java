...
public class NetworkService_backup extends Service{
	public NetworkService_backup(){
	}
			...
			public void run(){
				try{
					if(refreshNewMsg()==1){
						...
						MyTools.showToast(NetworkService_backup.this,"!Poor Network... :(",Toast.LENGTH_SHORT);
					}
				}catch(Exception e){
					e.printStackTrace();
				}
...
//		Socket socket = new Socket("specialchat.0x29a.cc", 21027);
	...
	private int refreshNewMsg() throws JSONException{
		...
		if(data == null){
			...
			ChatListSQLiteHelper clh=new ChatListSQLiteHelper(NetworkService_backup.this,"chat_list.db",1);
			for(int i=1;i<=new_msg_num;i++){ //todo may bug here
				...
				MsgSQLiteHelper mh=new MsgSQLiteHelper(NetworkService_backup.this,"msg_"+friend_id+".db",1);
	...
	private void showNewMsgNotification(String user_id,String nickname,String msg_content,String send_time) {
		NotificationManager notificationManager = (NotificationManager) NetworkService_backup.this.getSystemService(NOTIFICATION_SERVICE);
